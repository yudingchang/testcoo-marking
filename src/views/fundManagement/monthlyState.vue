<template>
  <div class="monthlyState">
   <el-row v-if="monthlyStateVif">
    <!-- 测库月结 -->
    <el-col :span="24" class="testcoo-repayment repayment-title">
      ...
      <div class="outAccount">
        ...
        <!-- outAccount-bill -->
        <div class="outAccount-bill" v-if="outAccountBill" v-for="(item,index) in outAccountBillData" :key="index">
    ...
    <el-col :span="24" class="testcoo-outstanding outstanding-title">
      ...
      <ul class="outstanding">
        <li>未出账</li>
        <li @click="OutstandingAccountDeVif">
          <span>更多</span>
          <span><i></i></span>
        </li>
      </ul>
...
import {outAccountData,ReimbursementDetailData,outstandingAccount} from '@/api/monthlyState'
export default{
  name: 'monthlyState',
  data(){
    return {
      ...
      //测库月结主页显示隐藏
      monthlyStateVif:true,

      //已出账显示隐藏
      outAccountBill:true,
      outAccountTotal:[], //已出账所有账单总额
      outAccountBillWarn:false, //日期靠近还款日时提醒用户
      outAccountRmbpay:true, //人民币还款
      outAccountPayFail:false, //还款失败提醒
      outAccountPaying:false, //还款中提醒
      outAccountBillData:[], //已出账数据
      monthlyBilldataId:"", //已出账单月账单ID
      outAccountPayMayF:"", //已出账单点击还款
      ...
      //单月账单还款详情页显示隐藏
      monthlyPayDetail:false,

      //测库月结已出账还款明细显示隐藏
      monthlyReimbursement:false,

      //测库月结已出账还款明细单月查看详情
      ReimbursementDetail:false,
      ReimbursementDetailDataId:'',//ReimbursementDetail 页面的数据的id

      //单月账单查看详情页面显示隐藏
      monthlyBillDetail:false,

      //测库月结未出账明细页面显示隐藏
      OutstandingAccountDe:false,
      ...
      ]  //monthlyState 月结未出账列表数据
    }
  },
  components:{monthlyPayDetail,monthlyBillDetail,monthlyReimbursement,ReimbursementDetail,OutstandingAccountDe},
  created(){
  ...
  methods:{
    ...
    //点击查看已还款
    checkAlreadyPay(){
      this.monthlyReimbursement=true
      this.monthlyStateVif=false
    },
    ...
    checkPayDetail(val){
      this.monthlyStateVif=false
      this.monthlyBillDetail=true
      this.monthlyBilldataId=val
      // console.log(val)
    },
    ...
    outAccountMoneyPay(val){
      this.monthlyStateVif=false
      this.monthlyPayDetail=true
      // console.log(this.$refs.p)
      // this.$refs.p.judgePayWay(val)
      this.outAccountPayMayF=val
    },

    //


    // monthly-reimbursement测库月结已出账还款明细传回来的data,根据数据中的ID发送请求
    ReimbursementData(a){
      this.ReimbursementDetail=true
      this.monthlyReimbursement=false
      console.log(a)
      this.ReimbursementDetailDataId=a
    },
    ...
    getOutstandingAccount(){
      outstandingAccount().then(response =>{
        if(response.data.code == 0){
          this.outstandingAccountTableData=response.data.list
        }
      })
    },
    ...
    //月结未出账明细
    OutstandingAccountDeVif(){
      this.OutstandingAccountDe=true
      this.monthlyStateVif=false
    },
